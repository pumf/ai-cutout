...
1. 下载对应架构的 dmg(Apple Silicon 选 arm64,Intel Mac 选 x64)
2. 打开 dmg,把应用拖到 **应用程序** 文件夹
3. **⚠️ 重要:首次打开前,先在终端执行一次** ⬇️

   ```bash
   sudo xattr -rd com.apple.quarantine /Applications/小飞AI抠图.app
   ```

   输入密码后再双击应用即可正常打开。

> **为什么需要这一步?**
> 应用未做 Apple 公证 (Notarization),macOS Sonoma/Sequoia 会把从互联网下载的未签名应用直接报为"已损坏"。上面这条命令是去掉 macOS 加的 quarantine 隔离属性,**不是真有损坏**。
> 你也可以右键点应用 → "打开" → 在弹框里点"打开"绕过 Gatekeeper,但部分新版本 macOS 已禁用该入口,直接 xattr 最稳。

如果运行后仍提示后端服务启动失败,可在 **系统设置 > 隐私与安全** 里手动允许打开,或在终端运行应用直接看错误日志。

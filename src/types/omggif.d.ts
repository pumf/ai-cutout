declare module 'omggif' {
  export class GifReader {
    ...
    height: number;
    numFrames(): number;
    frameInfo(frame_num: number): { x: number; y: number; width: number; height: number; delay: number };
  ...
  export class GifWriter {
    ...
    addFrame(x: number, y: number, width: number, height: number, data: Uint8Array, opts?: { delay?: number; palette?: number[]; transparent?: number; disposal?: number }): number;
    end(): number;
  }
}